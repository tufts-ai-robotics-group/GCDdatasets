...
from gcd_data.config import cub_root
...
def subsample_classes(dataset, include_classes=range(160)):
    ...
    # CUB classes are indexed 1 --> 200 instead of 0 --> 199
    include_classes_cub = np.array(include_classes) + 1
    cls_idxs = [x for x, (_, r) in enumerate(dataset.data.iterrows())
...
def get_cub_datasets(train_transform, test_transform, train_classes=range(160),
                     prop_train_labels=0.8, split_train_val=False, seed=0, download=True):
