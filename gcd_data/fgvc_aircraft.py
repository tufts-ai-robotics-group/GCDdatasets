...
from gcd_data.config import aircraft_root
...
class FGVCAircraft(Dataset):
    ...
    def download(self):
        ...
        download_url(self.url, parent_dir, tar_name)
...
def get_aircraft_datasets(train_transform, test_transform, train_classes=range(50),
                          prop_train_labels=0.8, split_train_val=False, seed=0):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = FGVCAircraft(
        root=aircraft_root, transform=train_transform, split='trainval')
...
if __name__ == '__main__':
    ...
    print(len(set([i[1] for i in x['train_labeled'].samples])))
    print('Printing total number of classes...')
    print(len(set([i[1] for i in x['train_unlabeled'].samples])))
